...
CREATE TABLE IF NOT EXISTS appliance (
    ...
    name varchar(100) not null,
    type_id bigint not null references type(type_id),
...
as select customer_id, appliance_pool.basket_id, appliance_pool.appliance_id, appliance.name appliance_name, type_id, type.name type_name, price, quantity, status
...
as select appliance.appliance_id, appliance.name appliance_name, type.name type_name, category.name category_name, appliance.price, stock.stock, stock.shop_id, shop.address, appliance.description appliance_description, type.description type_description, category.description category_description
from appliance join type using(type_id)
...
GRANT SELECT ON type TO slave_user;
