...
async def get_all_products():
    ...
    try:
        ...
        query = """
            SELECT appliance_id, appliance_name, type_name, category_name, price, stock, shop_id, address
...
async def appliance_unique_checking(appliance: Appliance):
    ...
    try:
        ...
        """
        type_id = await conn.fetchrow(type_query, appliance.type)
        category_query = """
            SELECT category_id FROM category WHERE name = $1
        """
        category_id = await conn.fetchrow(category_query, appliance.category)
        query = """
            SELECT appliance_id FROM appliance WHERE name = $1 AND type_id = $2 AND category_id = $3 AND price = $4
        """
        result = await conn.fetchrow(query, appliance.name, type_id['type_id'], category_id['category_id'], appliance.price)
...
async def appliance_in_shop_unique_checking(appliance: Appliance) -> bool:
    ...
    try:
        query = """
            SELECT appliance_id FROM appliance_with_shop WHERE appliance_name = $1 AND type_name = $2 AND address = $3 AND category_name = $4 AND price = $5
        """
        result = await conn.fetchrow(query, appliance.name, appliance.type, appliance.shop, appliance.category, appliance.price)
        if result is not None:
            return False
        return True
    finally:
...
async def add_new_product(appliance: Appliance):
    ...
    try:
        type_id_query = """
            SELECT type_id FROM type WHERE name = $1
        """
        type_id = await conn.fetchrow(type_id_query, appliance.type)
        category_id_query = """
            SELECT category_id FROM category WHERE name = $1
        """
        ...
        if await appliance_unique_checking(appliance):
            appliance_query = """
                INSERT INTO appliance (name, type_id, category_id, price, description) VALUES ($1, $2, $3, $4, $5)
            """
            await conn.execute(appliance_query, appliance.name, type_id['type_id'], category_id['category_id'], appliance.price, appliance.description)
        shop_id_query = """
            SELECT shop_id FROM shop WHERE address = $1
        """
        shop_id = await conn.fetchrow(shop_id_query, appliance.shop)
        appliance_id_query = """
            SELECT appliance_id FROM appliance WHERE name = $1 AND type_id = $2
        """
        appliance_id = await conn.fetchrow(appliance_id_query, appliance.name, type_id['type_id'])
...
async def get_all_information_about_product(appliance_id: int, shop_id: int):
    ...
    try:
        query = """
            SELECT appliance_id, appliance_name, type_name, category_name, price, stock, shop_id, address, appliance_description, type_description, category_description
            FROM appliance_with_shop
