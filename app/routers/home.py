...
from database.actions_with_types import get_all_types, add_new_type, type_unique_checking
...
async def add_new_type_query(request: Request,
                            type: Brand = Form()):
    if (len(type.description) == 0):
        type.description = None
    if not await type_unique_checking(type):
        return {"status": "FAIL", "message": "Такой бренд уже есть!"}
    else:
        await add_new_type(type)
